...
class Process {
    ...
    receive(predicate = () => true) {
        ...
        if (this.__messageBox.length > 0) {
            return new this.__context.Promise((resolve, reject) => {
                ...
                var index = this.__messageBox.findIndex(predicate);
                if (index !== -1) {
                    const [message] = this.__messageBox.splice(index, 1);
     ...
     */
    __give(message) {
        if (this.__state == ProcessStates.WAITING && this.__predicate(message)) {
            return this.__handleMessage(message);
        }
