...
const constructorRX =
    /^\$serialize:constructor:(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/;
const symbolRX = /^\$serialize:symbol:(.*)$/;
    ...
    switch (typeof value) {
        ...
        case "object":
            ...
            if (value.constructor)
                return maybeReplaceObjectStringify(value);
    }
    ...
    if (keys.length == 1 && constructorRX.test(keys[0])) {
        ...
        if (cons) {
            ...
            if (!json.prototype || json.prototype.constructor !== cons) {
                const result = Object.create(cons.prototype);
...
function getConstructorKey(ctor) {
    return registeredClasses.get(ctor);
}
